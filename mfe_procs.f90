...
module mfe_procs
  ...
  real(kind=wp), dimension(:,:,:), allocatable, private :: jaca, jacb, jacc
  ...
  contains
    ...
    subroutine check_soln (u, key, errc)
     ...
     !!! Check for bad elements, saving their lengths for ENORM.
      ...
      do j = 1, nelt
        ...
        dx(j) = u(x,j+1) - u(x,j)
        if (dx(j) < dxmin) then
         write (unit=log_unit,fmt="(/a,i4)") "** CHECK_SOLN: Bad element -- no. ", j
         errc = 1
         return
        end if
   ...
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    subroutine eval_residual (u, udot, t, r)
      ...
      real(kind=wp), dimension(:,:), intent(in)  :: u, udot
      real(kind=wp), dimension(:,:), intent(out) :: r
      real(kind=wp), intent(in)  :: t

      ! Local variables.
      integer  :: errc
      real(kind=wp), dimension(nepn,nepn,nnod) :: diag
     ...
     !!! EVALUATE THE RESIDUAL
      ...
      if (errc /= 0) then
        ...
        write (unit=log_unit, fmt="(/,a,i4)") "** EVAL_RESIDUAL: Bad element -- no. ", errc
        errc = 1
        return
   ...
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    subroutine eval_jacobian (u, udot, t, h, errc)
      ...
      if (errc /= 0) then
        ...
        write (unit=log_unit, fmt="(/,a,i4)") "** EVAL_JACOBIAN: Bad element -- no. ", errc
        errc = 1
        return
      ...
      if (errc /= 0) then
        ...
        write (unit=log_unit, fmt="(/,a,i4)") "** ELTFY: Bad element -- no. ", errc
        errc = 2
        return

      end if

      if (.not. allocated(jaca)) then
   ...
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    subroutine eval_udot (u, t, udot, errc)
      ...
      real(kind=wp), dimension(:,:), intent(in)  :: u
      real(kind=wp), dimension(:,:), intent(out) :: udot
      real(kind=wp), intent(in)  :: t
      integer, intent(out)  :: errc
      ...
      if (errc /= 0) then
        ...
        write (unit=log_unit, fmt="(/,a,i4)") "** EVAL_UDOT: Bad element -- no. ", errc
        errc = 1
        return
